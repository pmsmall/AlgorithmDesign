...
public class Graphics {
	...
	public Graphics(int n, int v, Scanner sc) {
		...
		for (int i = 0; i < n; i++) {
			...
			x = sc.nextDouble();
			y = sc.nextDouble();
			nodeList.add(new Node(title, message, x, y));
			sc.nextLine();
		}
		...
		for (int i = 0; i < v; i++) {
			...
			title = sc.nextLine();
			message = sc.nextLine();
			nodeList.get(n0).e.add(new Edge(n0, anotherNode, weight, title, message));
			nodeList.get(anotherNode).e.add(new Edge(anotherNode, n0, weight, title, message));
			edgeList.add(new Edge(n0, anotherNode, weight, title, message));
		}
	}
