...
public class MainFrame {
	...
	public MainFrame() {
		...
		MyButton typeChange = new MyButton(new itermButtonTheme("0-1背包", 16, 22) {
			@Override
			...
			public void checkCenterPanel() {
				if (centerPanel == null) {
					...
					MyLabel input_size_packet = new MyLabel("请输入背包容量");
					...
					MyLabel input_size_lable = new MyLabel("请输入物品的个数");
							...
							try {
								...
								con.addActionListener(new ActionListener() {
									...
									public void actionPerformed(ActionEvent e) {
										...
										JOptionPane.showConfirmDialog(null, result_string, "结果",
		...
		MyButton relocate = new MyButton(new itermButtonTheme("Dijksta", 16, 22) {
			...
			public void onClick(MouseEvent e) {
				if (points == null || linkPoints == null) {
					JOptionPane.showConfirmDialog(null, "请先生成图", "错误", JOptionPane.YES_NO_OPTION);
					return;
				}
				if (selectPoint == -1 || linkPoints == null) {
					JOptionPane.showConfirmDialog(null, "请先选择起点", "错误", JOptionPane.YES_NO_OPTION);
					return;
				}
				...
				JOptionPane.showConfirmDialog(null, node_list_string, "结果", JOptionPane.YES_NO_OPTION);
				...
				// }.start();
				// } else
				// JOptionPane.showConfirmDialog(null, "已经在排序", "错误",
				// JOptionPane.YES_NO_OPTION);
		...
		MyButton open = new MyButton(new itermButtonTheme("最优二叉", 16, 22) {
			@Override
			public void onClick(MouseEvent e) {
				String s = JOptionPane.showInputDialog("请输入点的值，用\',\'分割");
				double[] p;
				try {
					String[] tmp = s.split(",");
				...
				JOptionPane.showConfirmDialog(null, result, "结果", JOptionPane.YES_NO_OPTION);
		...
		MyButton binarySearch = new MyButton(new itermButtonTheme("n 皇后", 16, 22) {
			@Override
			public void onClick(MouseEvent e) {
				super.onClick(e);
				String s = JOptionPane.showInputDialog("请输入n");
				int n_n;
				try {
					n_n = new Integer(s);
				} catch (NumberFormatException nfe) {
				...
				JOptionPane.showConfirmDialog(null, result_string, "结果", JOptionPane.YES_NO_OPTION);
				...
				// theme.find(index);
				// } else
				// JOptionPane.showConfirmDialog(null, "输入数字没有找到", "错误",
				// JOptionPane.YES_NO_OPTION);
				// } catch (NumberFormatException e1) {
				// JOptionPane.showConfirmDialog(null, "输入数字不符合规定", "错误",
				// JOptionPane.YES_NO_OPTION);
				// }
			}
		});
